...
export function formatDateWithTime(date: string, dateFormat?: string) {
	return format(new Date(date), dateFormat ?? "yyyy-MM-dd HH:mm:ss");
}
...
export function formatDateRange(dates: TZDate[]): string {
	...
	if (dates.length === 1 || dates[0]?.getTime() === dates[1]?.getTime()) {
		return dates[0] ? formatFullDate(dates[0]) : "";
	}
