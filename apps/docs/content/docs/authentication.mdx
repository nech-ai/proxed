...
The following endpoints are intentionally public and do **not** require authentication headers:
...
*   The project-scoped model list endpoints only require the project ID in the URL (no auth headers).
*   These endpoints return the models supported by Proxed, plus display name and pricing metadata.
