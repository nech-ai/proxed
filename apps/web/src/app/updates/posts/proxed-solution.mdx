...
- iOS SDK with DeviceCheck integration
...
We protect your API keys using a unique split-key approach:
- Keys are never exposed in your mobile app
- Keys never travel over the network
- Your app only uses a project-specific identifier
- Even if someone decompiles your app, they get at most a fragment
- Our cloud never has your full provider key
...
- SDK handles token generation automatically
...
- Automatic cost prevention

<br />
### Easy Integration

From a developer's perspective, using Proxed.AI is straightforward:
1. Install our iOS SDK
2. Configure your project and endpoint
3. Replace direct API calls with Proxed.AI calls
4. Let us handle:
   - Device verification
   - Key management
   - Response formatting
